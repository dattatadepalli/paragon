...
() => {
  ...
  return (
    <div>
      ...
      <Button onClick={setOn} className="me-2" variant="outline-primary">set on</Button>
      <Button onClick={setOff} className="me-2" variant="outline-primary">set off</Button>
      <Button onClick={toggle} className="me-2" variant="outline-primary">toggle</Button>
    </div>
  );
}
```
